...
export function WeekendPlanner({
	...
	const [presences, setPresences] =
		useState<WeekendPresenceData[]>(weekendPresences);
	const [loadedMonths, setLoadedMonths] = useState<Set<number>>(new Set([0]));
	const [isPending, startTransition] = useTransition();

	const currentMonth = addMonths(new Date(), monthOffset);
	const weekends =
		monthOffset === 0 ? initialWeekends : getWeekendsForMonth(currentMonth);
							...
							return (
								...
								>
									...
									<div className="flex flex-col items-center gap-0.5">
										...
										/>
										<span
											className={`text-[10px] ${myPresence ? "text-green-600" : "text-gray-400"}`}
										>

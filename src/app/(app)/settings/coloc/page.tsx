...
export default async function ColocSettingsPage() {
	...
	const emergencyPermissions = isAdmin
		? await getEmergencyPermissions(member.colocation_id)
		: [];

	const devPermissions = isAdmin
		? await getDevPermissions(member.colocation_id)
		: [];
		...
		<div className="space-y-4 p-4">
			...
			{isAdmin && members && (
				<Card>
					...
					<CardContent>
						<EmergencyPermissionsManager
							members={members}
							permittedMemberIds={emergencyPermissions}
						/>
					</CardContent>
				</Card>
			)}

			{/* Permissions dev — admin only */}
			{isAdmin && members && (
				<Card>
					<CardHeader className="pb-2">
						<CardTitle className="flex items-center gap-2 text-sm">
